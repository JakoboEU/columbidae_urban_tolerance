...
```{r}
avenet_birdlife = read_csv('./avenet_birdlife.csv')
head(avenet_birdlife)
```
```{r}
...
```{r}
avenet_mapping = left_join(mapping, avenet_birdlife, by = c('species_name' = 'Species1'))
avenet_jetz_dups = avenet_mapping[, c('jetz_scientific_name', 'Beak.Length_Culmen', 'Beak.Length_Nares', 'Beak.Width', 'Beak.Depth', 'Tarsus.Length', 'Wing.Length', 'Kipps.Distance', 'Secondary1', 'Hand-Wing.Index', 'Tail.Length', 'Habitat.Density', 'Trophic.Niche', 'Primary.Lifestyle')]

avenet_jetz = avenet_jetz_dups %>% group_by(jetz_scientific_name) %>% summarise(
...
```
```{r}
col_traits_ut = left_join(clustered_species_ut, avenet_jetz)
col_traits_ut
```
