...
```{r}
urban_data = distinct(col_loc_jetz_mapping[col_loc_jetz_mapping$present_in_city == T,c('city_name', 'jetz_scientific_name', 'present_in_city')])
...
species_in_cities = tibble::column_to_rownames(species_in_cities, var='city_name')
picante_urban_community = as.matrix(species_in_cities)
```

```{r}
col_loc_jetz_mapping = left_join(col_loc, mapping)
regional_pool_data = distinct(col_loc_jetz_mapping[,c('city_name', 'jetz_scientific_name')])
regional_pool_data$present_in_pool = T

species_in_pools = pivot_wider(regional_pool_data[regional_pool_data$city_name %in% urban_data$city_name,], names_from = jetz_scientific_name, values_from = "present_in_pool", values_fill = list(present_in_pool = F))
species_in_pools = tibble::column_to_rownames(species_in_pools, var='city_name')
...
```{r}
t.test(community_change_df$pool, community_change_df$city, paired = TRUE, alternative = "two.sided")
```

```{r}
community_change_df$id - row_number(community_change_df$city)
...
ggplot(community_change_df_l, aes(x = type, y = value)) + geom_line(aes(group = id), size = 0.1) +  geom_boxplot() + xlab('Community') + ylab('MNTD') + labs(title = "MNTD Change between Urban and Regional Communities", subtitle = "t = -16.133, df = 742, p-value < 2.2e-16")
```
