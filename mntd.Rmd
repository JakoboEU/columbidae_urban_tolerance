...
```{r}
species_pool_cooccurance_pt1 = paired_presence %>% 
...
species_pool_cooccurance = left_join(species_pool_cooccurance_pt1, col_jetz_ut[,c('jetz_scientific_name', 'region_count', 'city_count', 'urban_tolerance')], by = c('species' = 'jetz_scientific_name'))
```

```{r}
species_pool_cooccurance[species_pool_cooccurance$species == 'Columba_oenas',]
```

```{r}
species_pool_cooccurance[species_pool_cooccurance$replaced > 0 & species_pool_cooccurance$held > 0,]
```
```{r}
species_pool_cooccurance$held_ratio = species_pool_cooccurance$held / species_pool_cooccurance$total_cities_paired
species_by_held_ratio = species_pool_cooccurance %>% filter(total_cities_paired > 5) %>% group_by(species, urban_tolerance) %>% summarise(average_held = mean(held_ratio))
ggplot(species_by_held_ratio, aes(x = average_held, y = urban_tolerance)) + geom_point() + geom_smooth(method="glm") + labs(subtitle = paste("Correlation", cor(species_by_held_ratio$average_held, species_by_held_ratio$urban_tolerance)))
```