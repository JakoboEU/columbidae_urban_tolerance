...
forest_paired_summary = forest_paired_to_city_data %>% group_by(cooccur) %>% summarise(mean = mean(pc1), sd = sd(pc1))
forest_paired_summary$lower_limit = forest_paired_summary$mean - forest_paired_summary$sd
forest_paired_summary$upper_limit = forest_paired_summary$mean + forest_paired_summary$sd

ggplot(forest_paired_to_city_data, aes(x = cooccur)) +
  geom_violin(aes(y = pc1)) + 
  geom_point(data = forest_paired_summary, aes(y = mean), color = "red") +
  geom_text(data = forest_paired_summary, aes(y = mean, label = round(mean, 1)), color = "red", nudge_x = 0.2) +
  geom_errorbar(data = forest_paired_summary, aes(ymin= lower_limit, ymax = upper_limit), color = "red", width = 0.2) +
  geom_text(data = forest_paired_summary, aes(y = lower_limit, label = round(lower_limit, 1)), color = "red", nudge_x = 0.2, size = 3) +
  geom_text(data = forest_paired_summary, aes(y = upper_limit, label = round(upper_limit, 1)), color = "red", nudge_x = 0.2, size = 3) +
  xlab('Species pair co-occur') + ylab('PC1') + labs(title='Forest Dwellers vs city landscape PC1', subtitle = 'chi-squared = 62.286, df = 1, p-value < 0.001')
```
